...
AtomicTools, and related contracts) originated as `eosio-contract-api`. AtomicAssets is an
open NFT standard, created by **Pink Network ([pink.gg](https://pink.gg))**, who also built
**AtomicHub**, the original AtomicAssets marketplace. Ownership of AtomicHub and this software
passed from Pink Network to **Spielworks** (Spielworks Markets GmbH), and then to **FACINGS**,
the current owner and maintainer. See `NOTICE` and `LICENSE` for copyright.
