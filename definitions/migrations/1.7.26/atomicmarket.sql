/*
  1.7.26 - Incremental, queue-driven update_atomicmarket_template_prices()
  ...
  * prio lanes (1.7.13): claim is ORDER BY prio, seq. Real-time trigger enqueues
    ride prio 0; the cutover seed (1.7.27) and every aging row ride prio 1 so a
  ...
  * atomicmarket_sales_filters_listed, whose MIN(price) over single-asset
  ...
  the cutover seed in 1.7.27, which includes every template present in
...
/*
  ...
  (1.7.25/atomicmarket.sql, `listing.updated_at_time` in the recompute select
...
-- The cutover seed is 1.7.27/atomicmarket.sql, deliberately not another
