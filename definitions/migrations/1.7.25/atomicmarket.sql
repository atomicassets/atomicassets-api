/*
  1.7.25 - Opt-in partition-parallel drain for large sales-filter backlogs.
  (Backport of the v2 line's 2.0.1, originally PR #71; SQL unchanged.)

  PRIORITY LANE. The body below says the recompute is verbatim from 1.7.11, and it
  is. What 1.7.11 did not have, and what postdates this drain's design, is 1.7.13's
  two-lane queue: the stock drain claims ORDER BY prio, seq so real-time enqueues
  jump the backlog. The partition claim here is ORDER BY seq only, so while workers
  are running the fast lane gets no priority. That does not affect what is written:
  the recompute is unchanged, so a partition-drained row is byte-identical to a
  stock-drained one, and because every path that sets prio also bumps seq, the
  release guard still refuses to delete a row re-enqueued mid-batch. It is a
  scheduling difference during a deliberate bulk drain, not a correctness one.
