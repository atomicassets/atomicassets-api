...
## What this costs on a populated database

The 1.x line is what mainnet runs, so this is the deployment where the tables are
large: `atomicmarket_buyoffers_assets` is around 10.6M rows and 1.9 GB on wax.

The two `ALTER TABLE ... ALTER COLUMN` statements are catalog writes and return
immediately. The two `ANALYZE` statements are the only real work, and they are
sampled rather than full scans, bounded by the columns' statistics target rather
than by table size. `ANALYZE` takes
`SHARE UPDATE EXCLUSIVE`, so readers and the filler's writes continue throughout;
it conflicts only with a concurrent `VACUUM`/`ANALYZE` on the same table.

The file runs as deferred SQL, outside the migration transaction and on the pool
that allows an hour per statement, so none of it inherits the 30 second cap the
in-transaction migration client runs under.
