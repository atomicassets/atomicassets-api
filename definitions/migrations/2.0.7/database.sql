/*
  2.0.7 - Cutover seed for the incremental template-price queue
  (the queue, triggers and drain are 2.0.6; the seed itself is atomicmarket.sql).

  WHY THE SEED IS ITS OWN VERSION
  2.0.6 replaces the enqueue triggers, and DROP TRIGGER takes ACCESS EXCLUSIVE
  ...
  Split across two versions, 2.0.6 commits as soon as its DDL is done and the
  ...
  Same constraint as 2.0.6/database.sql: database.sql runs on every deployment,
  ...
  The timeouts repeat 2.0.6's treatment and cover the whole version transaction.
...
UPDATE dbinfo SET "value" = '2.0.7' WHERE name = 'version';
