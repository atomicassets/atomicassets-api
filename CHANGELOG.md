...
Carries a template's mutable data into every asset response, makes the template data filters and the template name search read it, and serves the AtomicAssets v2 action logs the filler already records.

### Upgrading

- The migration set moves to `2.0.8`. It replaces the `atomicassets_assets_master` view and builds two indexes on `atomicassets_templates`, and the filler applies all three on boot.
