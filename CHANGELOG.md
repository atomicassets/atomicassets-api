...
Carries a template's mutable data into every asset response, the template data filters and the template name search, serves the AtomicAssets v2 action logs the filler already records, records the cancel the AtomicMarket v2 contract performs on a legacy bundle listing instead of a trade that never happened, lets `ip_whitelist` name address ranges so a fleet behind a private network can skip the rate limiter, adds `peer_whitelist` for callers matched on the connection's own peer address, and warns at startup when `trust_proxy` lets a forwarded header reach `ip_whitelist`.
...
- The migration set moves to `2.0.9`. `2.0.8` replaces the `atomicassets_assets_master` view and builds two indexes on `atomicassets_templates`, `2.0.9` adds one metadata-only column, and the filler applies them on boot.
- The view replacement rewrites no data. A view holds no rows, so its cost is a catalog update that does not scale with the size of `atomicassets_assets`, and the new expressions sit inside an existing `json` column, so `CREATE OR REPLACE` keeps the signature and `atomicmarket_assets_master` stays valid without a rebuild.
- Both indexes on `atomicassets_templates.mutable_data` are built `CONCURRENTLY` from the version's deferred file, after the migration transaction commits, so neither takes a lock that blocks writes to that table. Each serves one of the two comparisons the template filters make: a `jsonb_ops` GIN index for containment, matching `atomicassets_templates_immutable_data_gin`, and a trigram GIST index on the extracted name for `ILIKE` and the `<%` word-similarity operator, matching `atomicassets_templates_name`. Postgres builds a `BitmapOr` only when every arm of the condition is indexable, so leaving either out would cost a sequential scan of the whole templates table rather than a slower arm. Both scan the table to build, and the filler awaits the deferred lane, so a large deployment stays down for the length of those builds. A cancelled build leaves an invalid index that the guarded statement then skips, so check both after upgrading with the query in `definitions/migrations/2.0.8/README.md`.
- The repair covers the v1 asset and template endpoints, including the `/v1/burns` filters and the name sort on the v1 market listings. `/atomicmarket/v2/sales` is unchanged: its name search and data filters resolve from the denormalized `atomicmarket_sales_filters`, whose three-way merge excludes template mutable data, and correcting that needs a full re-drain of a multi-million-row partitioned table.
- A consumer reading `data` on an asset sees keys it did not see before, wherever the template holds them mutably. The layer order is unchanged: the template's immutable values still outrank the asset's own, and its mutable values remain the fallback the asset overrides. `sort=name` reads the same four layers, so an asset naming itself only through its template's mutable data now sorts on that name instead of last.
...
- Migration `2.0.9` adds `atomicmarket_config.v2_marker_block`, the last block a market contract still records under the pre-v2 rules. The `ALTER TABLE` is metadata-only and the backfill touches one row per market contract, so the migration is instant. A deployment already indexing a v2 chain has the marker set to the furthest position held by a reader whose last checkpoint was at the chain head, which turns the rules on for the blocks still ahead and leaves the recorded history alone. A reader still catching up, or one whose position was rewound before the upgrade, keeps a null marker until it observes the flip delta, and a chain still on v1 keeps a null marker until its reader observes the flip.

### Bug fixes

- Asset responses carry `template.mutable_data`, `template.deleted_at_time` and `template.deleted_at_block`. The nested `template` object of `atomicassets_assets_master` listed `immutable_data` alone, so all three were documented in the OpenAPI document and never populated: `template.mutable_data` was always `{}`, the merged `data` dropped every key a template holds mutably, and a template deleted on chain read the same as a live one. `atomicmarket_assets_master` opens with `SELECT asset.*`, so every market listing carried the same gap and inherits the repair. (#204)
- The `data.*` and `template_data.*` filters, and the `match` and `search` template name filters, read `atomicassets_templates.mutable_data` alongside `immutable_data` on the template, asset and burn endpoints. A value a collection stores mutably on its templates was reachable through no filter at all: a template holding a mutable `lore` of `origin` was returned by neither `template_data.lore=origin` nor `match=origin`. Each requested pair is satisfied by either column on its own, so a filter naming one immutable key and one mutable key matches. (#204)
...
- A RAM payer change left no record the API could serve. Both v2 actions that move it, `setrampayer` and `setlastpayer`, emit `logrampayer` inline, and no handler read that trace. The filler now stores it with the asset id, the asset owner and both payers, and `/v1/assets/{asset_id}/logs` serves it. No column and no migration carry the payer; the trace is the whole record. (#205)
- A listing created before the v2 upgrade may hold more than one asset, and the v2 contract cancels such a listing instead of settling it: `purchasesale` erases the sale and charges the buyer nothing, `auctionbid` and both auction claims dissolve an auction neither side has claimed, and `acceptbuyo` refunds the buyer exactly like `declinebuyo`. The filler recorded each of those as a completed trade, so a bundle nobody could buy became a sale with a buyer and a price, an auction gained a bid that was refunded in the same transaction, and a buyoffer read as accepted. All five actions now record the cancel, the dissolution or the refund the contract performed, while a partially claimed bundle auction still records the normal claim and a chain on a v1 contract keeps the old recording. (#206)
- An ended legacy bundle auction that nobody had claimed reported `state` 3, sold, from `/atomicmarket/v1/auctions`. That auction can never settle, because a claim on a v2 contract refunds the bid and returns the assets, so it now reports `state` 4, invalid, and a query filtered on either state agrees with the formatted row. An auction on a v1 contract keeps the old state. (#206)

### Other changes

- `ip_whitelist` in `server.config.json` accepts CIDR ranges as well as exact addresses, in IPv4 or IPv6 notation. The list is compiled once at startup with the matcher express uses for `trust proxy`, and both the rate limiter and the response cache consult the same predicate. An absent `ip_whitelist` key is treated as an empty list; before, it made the rate limiter throw on the first request. A second key, `peer_whitelist`, takes the same entries and matches the TCP peer address of the connection, `req.socket.remoteAddress`, which no forwarded header can alter; a request is whitelisted when either list matches. When `ip_whitelist` is non-empty and `trust_proxy` is `true` or a hop count, the server logs a warning at startup: `req.ip` then derives from a forwarded header a client can supply, so a client that forges the header takes any `ip_whitelist` entry, and directly connected callers belong in `peer_whitelist`. (#201)
