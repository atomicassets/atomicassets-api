...
export default class AtomicMarketHandler extends ContractHandler {
    ...
    async register(processor: DataProcessor, notifier: ApiNotificationSender): Promise<() => any> {
        ...
        // get_table_rows on atomicmarket.{tbuyo,buyoffers,sales,auctions} every 30 min
        // as a drift canary; upstream RPC node stopped accepting the queries
        // ("Invalid name at /v1/chain/get_table_rows") and the job has been logging
        // errors ever since, tripping the status.atomichub.io Marketplace Data
        // Sync indicator. Event-based filler ingest is authoritative; reconcile
        // method bodies (reconcileListings + reconcileListingType) retained below
        // for easy re-enable if we ever want to reintroduce a drift check.
