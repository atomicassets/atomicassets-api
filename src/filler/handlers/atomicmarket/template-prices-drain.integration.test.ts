...
// Integration coverage for the 2.0.6 incremental, queue-driven
...
describe('template-prices queue - enqueue triggers (2.0.6)', () => {
...
describe('update_atomicmarket_template_prices - batched drain (2.0.6)', () => {
...
describe('update_atomicmarket_template_prices - authoritative per-template recompute (2.0.6)', () => {
...
describe('template-prices migration - replay, seed and storage', () => {
    ...
    txit('dbinfo reports at least the 2.0.6 schema version', async () => {
        const res = await client.query<{ value: string }>('SELECT "value" FROM dbinfo WHERE name = $1', ['version']);

        expect(compareVersionString(res.rows[0].value, '2.0.6')).to.be.at.least(0);
    });
...
describe('drainAtomicmarketTemplatePrices - TS batch loop against the real drain (2.0.6)', () => {
